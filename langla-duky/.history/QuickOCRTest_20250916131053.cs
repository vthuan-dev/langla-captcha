    ...
    public class QuickOCRTest
    {
        public static void RunQuickTest()
        {
            ...
            try
            {
                ...
                // TesseractEngine yêu cầu đường dẫn trực tiếp đến thư mục 'tessdata'.
                string tessdataPath = Path.Combine(baseDirectory, "tessdata");
                if (!Directory.Exists(tessdataPath))
                {
                    string errorMsg = $"Lỗi: Không tìm thấy thư mục tessdata tại '{tessdataPath}'";
                    Console.WriteLine(errorMsg);
                    MessageBox.Show(errorMsg, "Lỗi Tesseract", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
                Console.WriteLine($"Đã tìm thấy thư mục tessdata: {tessdataPath}");

                // Chạy OCR
                // SỬA LỖI: Tải ảnh trực tiếp vào định dạng Pix của Tesseract
                // để tránh lỗi Heap Corruption (0xc0000374) có thể xảy ra
                // khi chuyển đổi từ đối tượng Bitmap của System.Drawing.Common.
