    ...
    public partial class MainForm : Form
    {
        ...
        public MainForm()
        {
            ...
            btnQuickTest.Click += (s, e) =>
            {
                try
                {
                    LogMessage("🔍 Đang chạy Quick OCR Test với hình ảnh mẫu...");
                    
                    // Sử dụng QuickOCRTest để test OCR với hình ảnh mẫu
                    Task.Run(() => 
                    {
                        try
                        {
                            LogMessage("📁 Kiểm tra đường dẫn hình ảnh...");
                            
                            // Kiểm tra đường dẫn hình ảnh trước
                            ...
                            foreach (var path in possiblePaths)
                            {
                                ...
                                {
                                    imagePath = path;
                                    this.Invoke(new Action(() => LogMessage($"✅ Tìm thấy hình ảnh: {path}")));
                                    break;
                                }
                            }
                            ...
                            if (imagePath == null)
                            {
                                this.Invoke(new Action(() => LogMessage("❌ Không tìm thấy hình ảnh ocr-1.png")));
                                return;
                            }
                            
                            // Chạy QuickOCRTest với logging chi tiết
                            this.Invoke(new Action(() => LogMessage("🔤 Bắt đầu thực hiện OCR...")));
                            
                            // Wrapper an toàn để tránh crash
                            try
                            {
                                ...
                                using (var sw = new StringWriter())
                                {
                                    ...
                                    try
                                    {
                                        // Chạy OCR test an toàn
                                        QuickOCRTest.RunQuickTest();
                                        Console.WriteLine("✅ OCR Test hoàn thành");
                                    }
                                    catch (Exception ex)
                                    {
                                        Console.WriteLine($"❌ Lỗi trong QuickOCRTest: {ex.Message}");
                                    ...
                                    this.Invoke(new Action(() => 
                                    {
                                        ...
                                        // Cập nhật UI
                                        _lblCaptcha.Text = "Captcha: Xem log để biết kết quả";
                            ...
                            catch (Exception ocrEx)
                            {
                                this.Invoke(new Action(() => 
                                {
                                    LogMessage($"❌ Lỗi OCR nghiêm trọng: {ocrEx.Message}");
                                    LogMessage($"📋 Loại lỗi: {ocrEx.GetType().Name}");
                        ...
                        {
                            this.Invoke(new Action(() => LogMessage($"❌ Lỗi trong QuickOCRTest: {ex.Message}")));
                            this.Invoke(new Action(() => LogMessage($"📋 Stack trace: {ex.StackTrace}")));
                        }
                    });
                }
                catch (Exception ex)
                {
                    LogMessage($"❌ Lỗi Quick OCR Test: {ex.Message}");
                }
            };
                    ...
                    using (Bitmap captchaCrop = new Bitmap(captchaArea.Width, captchaArea.Height))
                    {
                        ...
                        else
                        {
                            ...
                            string debugInfo = $"Captcha size: {captchaCrop.Width}x{captchaCrop.Height}\n" +
                                              ...
                                              $"Timestamp: {timestamp}\n" +
                                              $"Game window: {_selectedGameWindow?.WindowTitle}";
